...
public class PointInfo {
    ...
    protected void set(int numPoints, float[] x, float[] y, float[] pressures,
        ...
        if (isMultiTouch) {
            xMid = (x[0] + x[1]) * .5f;
            yMid = (y[0] + y[1]) * .5f;
            pressureMid = (pressures[0] + pressures[1]) * .5f;
            dx = Math.abs(x[1] - x[0]);
            dy = Math.abs(y[1] - y[0]);
        } else {
            //single touch event
